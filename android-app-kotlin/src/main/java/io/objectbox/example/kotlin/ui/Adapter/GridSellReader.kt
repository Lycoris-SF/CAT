...
class GridSellReader(private val context: Context, private var item: Item?) :
    RecyclerView.Adapter<GridSellReader.ViewHolder>() {
        ...
        if (position == 0) {
            ...
            holder.textView1.setOnLongClickListener {
                val searchQuery = ObjectBoxSC.boxStore.boxFor(Item::class.java)
                    .query().equal(Item_.title, currentSell, QueryBuilder.StringOrder.CASE_INSENSITIVE).build()
